            ...
            <ul className="hidden md:flex items-center list-none gap-6">
                {NAV_LINKS.map(({ href, key }) => (
                    <li key={href}>
                        <Link
                            href={href}
                            className={`block py-1.5 text-[15px] no-underline transition-colors duration-200 font-medium ${key === "blog"
                    ...
                    <ul className="flex flex-col py-4 px-6 space-y-4">
                        {NAV_LINKS.map(({ href, key }) => (
                            <li key={href}>
                                <Link
                                    ...
                                    className={`block text-[16px] font-medium transition-colors ${key === "blog"
                                        ? "text-blue-600 hover:text-blue-700"
                        ...
                        <li className="pt-4 mt-2 border-t border-gray-100 flex items-center justify-between">
                            ...
                            <Link
                                ...
                                onClick={() => setIsMobileMenuOpen(false)}
                                className="inline-block text-center bg-gray-900 text-white text-[15px] font-semibold py-2.5 px-5 rounded-full"
                            >
                                {t("openTool")}
                            </Link>
                        </li>
