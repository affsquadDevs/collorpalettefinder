...
export async function generateMetadata({ params }: { params: Promise<{ room: string }> }): Promise<Metadata> {
    ...
    const g = getRoomGuide(room);
    if (!g) {
        return { title: "Page Not Found — colorPaletteFinder" };
    }
    const url = `${SITE_URL}/interior-color-palettes/${g.slug}`;
    return {
        title: g.metaTitle,
...
export default async function RoomPage({ params }: { params: Promise<{ room: string }> }) {
    ...
    const palettes = INTERIOR_PALETTES.filter((p) => p.room === g.room);
    const others = INTERIOR_ROOM_GUIDES.filter((r) => r.slug !== g.slug);
    const url = `${SITE_URL}/interior-color-palettes/${g.slug}`;
    ...
    const breadcrumbSchema = {
        ...
        itemListElement: [
            ...
            { "@type": "ListItem", position: 2, name: "Interior Color Palettes", item: `${SITE_URL}/interior-color-palettes` },
            { "@type": "ListItem", position: 3, name: `${g.room} Color Schemes`, item: url },
        ],
    };
            ...
            <header className="px-6 sm:px-10 lg:px-16 pt-12 pb-14 lg:pb-16 text-center border-b border-gray-200 bg-white">
                <div className="max-w-3xl mx-auto">
                    <nav aria-label="Breadcrumb" className="flex flex-wrap items-center justify-center gap-2 text-sm text-gray-500 mb-6">
                        ...
                        <Link href="/interior-color-palettes" className="hover:text-blue-600">Interior Color Palettes</Link>
                        <span className="text-gray-300">/</span>
                        <span className="text-gray-700">{g.room}</span>
                    </nav>
                    <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight mb-6">
                        {g.room} <span className="text-blue-600">Color Schemes</span>
                    </h1>
            ...
            <section className="px-6 sm:px-10 lg:px-16 pt-14 lg:pt-20">
                <div className="max-w-6xl mx-auto">
                    <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-2 text-center">
                        {g.room} palettes to start from
                    </h2>
                    <p className="text-gray-500 text-center max-w-2xl mx-auto mb-12">
                        Livable, harmonious schemes for the {g.room.toLowerCase()}. Click any swatch to copy its hex code.
                    </p>
            ...
            <section className="px-6 sm:px-10 lg:px-16 pt-16 lg:pt-20">
                <article className="max-w-3xl mx-auto">
                    ...
                    <section className="mt-16">
                        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-6">Color schemes for other rooms</h2>
                    ...
                    <div className="mt-14 p-6 bg-blue-50 border border-blue-100 rounded-xl text-center">
                        ...
                        <p className="text-gray-600 mb-4">Drop a color you love into the generator, pick a harmony rule, and check the contrast before you commit a single can of paint.</p>
